...
| メニュー時/停止時非表示 | メニュー/ポーズ中、または走行中でも完全停止 (アクセル/ブレーキ/ステアリング無操作 + 車速・水平加速度がほぼ 0) で同様にフェードアウト | OFF |
...
なお、テレメトリ (UDP パケット) が直近 500ms 以上届いていない場合は「未接続」とみなし、自動非表示は一時的に解除されてタイトルバーに `No telemetry` と表示されます。FH6 を起動していない / IP・ポートを間違えている / ファイアウォールでブロックされている、などの状況でウィジェットが消えて理由がわからなくなることを防ぎます。
